...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None):
                #dowmsample确定是否下采样
				#inplanes为上一个block的输出维度，即当前block的输入通道数
				#planes为该block的第一层卷积的输出通道数
				#base_width为整个网络的基通道数即64
        super(Bottleneck, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        width = int(planes * (base_width / 64.)) * groups  ##当前block的第一层卷积层的输出通道数
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False): #planes为每个stage第一层卷积层输出对应的通道数，blocks为每个block对应的bottleneck的个数
